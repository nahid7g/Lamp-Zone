...
const Inventory = () => {
  ...
  useEffect(() => {
    const url = `https://lamp-zone-server.vercel.app/parts/${id}`
    fetch(url)
      .then((res) => res.json())
  ...
  const onSubmit = (data) => {
    ...
    fetch('https://lamp-zone-server.vercel.app/orders', {
      method: 'POST', // or 'PUT'
      headers: {
            ...
            <div className='card-body'>
              <form onSubmit={handleSubmit(onSubmit)}>
                ...
                <div className='form-control'>
                  ...
                  <input
                    type='text'
                    placeholder='Minimum order must be 5000'
                    className='input input-bordered'
                    required
                    {...register('totalOrder', {
                      required: true,
                  ...
                  />
                  {errors.totalOrder?.type === 'validate' && (
                    <span className='label-text-alt text-red-500'>
                      You can not order less than 5000 items.
                    </span>
                  )}
                </div>
                <div className='form-control'>
                ...
                <button
                  type='submit'
                  disabled={errors.totalOrder?.type === 'validate'}
